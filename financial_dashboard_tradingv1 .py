...
if choice_strategy == choices_strategies[0]:
...
elif choice_strategy == choices_strategies[1]:  # RSI 交叉策略
    with st.expander("<策略參數設定>: RSI 參數"):
        MoveStopLoss = st.slider('選擇程式交易停損量', 0, 100, 30, key='MoveStopLoss_RSI')
        LongRSIPeriod_trading = st.slider('設定長RSI的K棒週期數目', 1, 100, 10, key='trading_RSI_long')
        ShortRSIPeriod_trading = st.slider('設定短RSI的K棒週期數目', 1, 100, 2, key='trading_RSI_short')
        Order_Quantity = st.slider('選擇購買數量', 1, 100, 1, key='Order_Quantity_RSI')
        KBar_df['RSI_long'] = Calculate_RSI(KBar_df, LongRSIPeriod_trading)
        KBar_df['RSI_short'] = Calculate_RSI(KBar_df, ShortRSIPeriod_trading)
    ...
    with st.expander("<策略參數設定>: 布林通道參數"):
        MoveStopLoss = st.slider('選擇程式交易停損量', 0, 100, 30, key='MoveStopLoss_BB')
        BB_period_trading = st.slider('設定布林通道週期', 1, 100, 20, key='trading_BB_period')
        BB_std_trading = st.slider('設定布林通道標準差倍數', 1, 5, 2, key='trading_BB_std')
        Order_Quantity = st.slider('選擇購買數量', 1, 100, 1, key='Order_Quantity_BB')

        KBar_df = Calculate_Bollinger_Bands(KBar_df, BB_period_trading, BB_std_trading)
    ...
    with st.expander("<策略參數設定>: MACD 參數"):
        MoveStopLoss = st.slider('選擇程式交易停損量', 0, 100, 30, key='MoveStopLoss_MACD')
        fast_period_trading = st.slider('設定 MACD 快速線週期', 1, 100, 12, key='trading_MACD_fast')
        slow_period_trading = st.slider('設定 MACD 慢速線週期', 1, 100, 26, key='trading_MACD_slow')
        signal_period_trading = st.slider('設定 MACD 訊號線週期', 1, 100, 9, key='trading_MACD_signal')
        Order_Quantity = st.slider('選擇購買數量', 1, 100, 1, key='Order_Quantity_MACD')

        KBar_df = Calculate_MACD(KBar_df, fast_period_trading, slow_period_trading, signal_period_trading)
