...
if choice_strategy == choices_strategies[0]:
    ...
    MoveStopLoss = st.slider('設定移動停損幅度 (元或點數)', 0, 100, 30, key=f"MoveStopLoss_{choice_strategy}")
    Order_Quantity = st.slider('設定下單數量', 1, 10, 1, key=f"OrderQty_{choice_strategy}")

    if 'Order_Quantity' not in locals():
    st.error("錯誤：請先設定策略參數中的『下單數量』")
    st.stop()
